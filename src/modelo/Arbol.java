...
public class Arbol<t> {
	...
	//ELIMINAR RECURSIVO -- adaptarlo a este tipo de Arbol
	   ...
	   public void eliminar(t dato, Comparator<t> cmp) {
		   nodoRaiz = eliminarRec(nodoRaiz, dato, cmp);
	   }

	private INodo<t> eliminarRec(INodo<t> nodo, t dato, Comparator<t> cmp) {
		if (nodo == null) return null;

		int comparacion = cmp.compare(dato, nodo.getDato());

		if (comparacion < 0) {
			nodo.setIzquierdo(eliminarRec(nodo.getIzquierdo(), dato, cmp));
		} else if (comparacion > 0) {
			nodo.setDerecho(eliminarRec(nodo.getDerecho(), dato, cmp));
		} else {
			...
			nodo.setDerecho(eliminarRec(nodo.getDerecho(), sucesor.getDato(), cmp));
		}
		return nodo;
	}
